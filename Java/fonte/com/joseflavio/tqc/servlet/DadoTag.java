 ...
 * @version 2013
 * @deprecated Devido a {@link DadoControle#imprimir(HttpServletRequest, TomaraQueCaia, Viagem, Informacao, Dado, String, java.io.Writer)}.
 */
public class DadoTag extends TQCTagSupport {
	...
	public int doEndTag() throws JspException {
		...
		try{
			...
			if( ! dado.isVisivel() ) return EVAL_PAGE;
			
			int indice = info.getIndice( dado );
			DadoControle controle = proc().getControle( dado );
			
			controle.imprimir( request, tqc, viagem, info, dado, TomaraQueCaiaDesktopServlet.PREFIXO_DADO + indice, out );
