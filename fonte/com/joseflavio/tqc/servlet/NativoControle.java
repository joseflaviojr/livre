 ...
 * @version 2013
 * @deprecated {@link Informacao#setPersonalizada(boolean)}
 */
class NativoControle implements DadoControle {
	...
	public void imprimir( HttpServletRequest request, TomaraQueCaia tqc, Viagem viagem, Informacao informacao, Dado dado, String id, Writer saida ) throws IOException {
		...
		Nativo nativo = (Nativo) dado;
		
		Map<Object, Object> prop = nativo.getPropriedades();
		if( prop == null ) return;

		String tipo = (String) prop.get( "tipo" );
		if( tipo == null ) return;
		
		if( tipo.equals( "iframe" ) ){
			
			String src = (String) prop.get( "src" );
			if( src == null ) return;
			
			saida.write( "<iframe src=\"" + src + "\" width=\"100%\" height=\"100%\" frameborder=\"0\" scrolling=\"auto\"></iframe>" );
