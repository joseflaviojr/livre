 ...
 * @version 2013
 */
class BinarioControle implements DadoControle {
	...
	public void transformar( HttpServletRequest request, TomaraQueCaia tqc, String conteudo, Dado destino ) {
		...
		Binario binDado = (Binario) destino;
		
		binDado.setValor( conteudo != null && conteudo.length() > 0 && conteudo.equals( "true" ) );
	...
	public String transcrever( HttpServletRequest request, TomaraQueCaia tqc, Dado dado ) {
		...
		Binario binDado = (Binario) dado;
		
		return binDado.isVerdadeiro() ? "true" : "false";
	...
	public void imprimir( HttpServletRequest request, TomaraQueCaia tqc, Viagem viagem, Informacao informacao, Dado dado, String id, Writer saida ) throws IOException {
		...
		if( binDado.isEditavel() ){
			
			saida.write( "<input type=\"checkbox\" id=\"" + id + "\" name=\"" + id + "\" value=\"true\"" );
			
			if( binDado.isVerdadeiro() ) saida.write( " checked" );
			
			saida.write( " />" );
			
		}else{
			
			String imagem = binDado.isVerdadeiro() ? TomaraQueCaia.ICONE_PEQUENO_CHECADO : TomaraQueCaia.ICONE_PEQUENO_NAO_CHECADO;
			saida.write( "<img src=\"" + imagem + "\" border=\"0\">" );
			
		}
		
		String rotulo = binDado.getRotulo();
		if( rotulo != null ) saida.write( " <span class=\"dadoBinarioTexto\">" + rotulo + "</span>" );
